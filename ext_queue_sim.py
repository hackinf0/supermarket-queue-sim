...
class Queues(Simulation):
    ...
    def __init__(self, lambd, mu, n, d,weibull_shape_arrival,weibull_shape_service,use_lwl):
        super().__init__()
    ...
    def schedule_completion(self, job_id, queue_index): 
        ...
        service_time = self.job_sizes[job_id] #taking back the size of the job
    ...
    def queue_work(self,i):
        ...
        """Return the total work left in queue i, 
            including the remaining time of the job currently being processed and the 
            full processing times of all jobs waiting in the queue.
        """
...
class Arrival(Event):
    ...
    def process(self, sim: Queues):   
        ...
        sim.job_sizes[self.id]=sim.service_gen()
...
def run_simulation(lambd, d, n, max_t, weibull_shape_arrival=1, weibull_shape_service=1, use_lwl=False):
    sim = Queues(lambd, 1, n, d, weibull_shape_arrival, weibull_shape_service,use_lwl)
    sim.run(max_t)
