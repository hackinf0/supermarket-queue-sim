...
def run_simulation(lambd, d, n, max_t):
    sim = Queues(lambd, 1, n, d)
    sim.run(max_t)
...
def plot_all(n, max_t, d):
    lambdas = [0.5, 0.9, 0.95, 0.99]
    fig, ax = plt.subplots(figsize=(8, 6))
    for lambd in lambdas:
        fractions = run_simulation(lambd, d, n, max_t)
...
if __name__ == '__main__':
    ...
    """
    On observe que l'augmentation de d — le nombre de files inspectées à l'arrivée d'un job — 
    réduit de façon spectaculaire la longueur des files. Avec d = 1 (affectation à une file tirée au hasard), 
    la fraction de files ayant au moins x jobs décroît lentement : à forte charge (λ = 0,99), 
    des files de longueur 14 restent fréquentes. Dès d = 2, la décroissance devient nettement plus rapide, 
    et pour d = 5 puis d = 10, elle est quasi verticale : les files dépassent rarement 2 ou 3 jobs, 
    même lorsque le système est chargé à 95 %.
    """